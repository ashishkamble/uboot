	...
	while (timer1 > 0) {
		...
		for (i=0; i<100; ++i) {
			...
			if ((BootType <= '0' || BootType > '6') && (BootType != '7') && (BootType != '8') && (BootType != '9'))
				BootType = '3';
			printf("\n\rYou choosed %c\n\n", BootType);
			if(BootType != '3' && BootType != '6' && BootType != '1')
			{
				printf("\n\rPlease verify yourself. Enter security Password \n\n");
				char password[20]="\0";
				int retry=0;
				char auth='\0';
				int v=0;
				...
				{
					while(v<20 && (auth=getc())!='\r')
					{
						password[v]=auth;
						v++;
					}
					if(strcmp(password,"fargo")==0)
					{
						printf("\n\rVerified \n\n");
						break;
					}
					...
					{
						printf("\n\rAuthentication failed. Please Enter correct password. %d retry left\n\n",2-retry);
...
/*
	char bootcausest[20];   
    strcpy(bootcausest,getenv("bootcause"));
    if(bootcauseval==1){
        printf("\n\rBootcause = watchdogreset:Oldbootcause=%s\n\n", bootcausest);
        if(bootcausest == NULL || strcmp("watchdogreset",bootcausest )!=0){
             setenv("bootcause", "watchdogreset");
            saveenv();
       }
   } else if(bootcauseval==2){
        printf("\n\rBootcause = softsystemreset:Oldbootcause=%s\n\n", bootcausest);
        if(bootcausest == NULL || strcmp("softsystemreset",bootcausest )!=0){
            setenv("bootcause", "softsystemreset");
             saveenv();
         }
    }else if(bootcauseval==3){
        printf("\n\rBootcause = softcpureset:Oldbootcause=%s\n\n", bootcausest);
        if(bootcausest == NULL || strcmp("softcpureset",bootcausest )!=0){
            setenv("bootcause", "softcpureset");
             saveenv();
        }
    }else{
        printf("\n\rBootcause = hardreset:Oldbootcause=%s\n\n", bootcausest);
         if(bootcausest == NULL || strcmp("hardreset", bootcausest )!=0){
            setenv("bootcause", "hardreset");
            saveenv();
         }
    }
*/
